...
def recommend_answer_ssvep(
    ...
    combined_df = pd.concat([fp1_df, fp2_df], axis=1) #fp1, fp2 데이터프레임 결합
    
    ########각 주파수의 max값으로 SSVEP 검출##########
    max_values = combined_df.max() #각 열 별 max값 탐색
    '''
    예시:
        27    2.266861
        13    2.097664
        ...
        23    2.528315
    '''
    max_column_name = max_values.idxmax() #제일 큰 max값의 주파수가 무엇인지 탐색
    '''
    예시: 23
    '''
    print('주파수 검출법: 최댓값')
    print('max 값 value들')
    print(max_values)
    print('max 값 주파수', max_column_name)


    ########각 주파수의 값의 누적합으로 SSVEP 검출##########
    sum = combined_df.sum(axis=0) #각 열 별 누적합 구하기
    '''
    예시:
         27    39.679190
         13    47.636882
         ...
         23    41.185298
    '''
    sum_max = sum.idxmax() #제일 큰 누적합의 주파수가 무엇인지 탐색
    '''
    예시: 13
    '''   
    print("주파수 검출법: 주파수 면적")
    print('주파수 sum 값 value들')
    print(sum)
    print('주파수 sum 값 주파수', sum_max)
    
    #누적합이 가장 큰 주파수 알고리즘을 SSVEP 검출 알고리즘으로 채택
    for i in range(len(frequencies)):
        if frequencies[i] == int(float(sum_max)): # 이 부분만 수정 #frequencies 요소의 값이 누적합이 가장 큰 주파수와 같을 때
            image_num = i*2+1
